...
def parse_llm_response(response: str) -> Dict[str, Any]:
    """LLM 응답 파싱 (이전과 동일)."""
    # 이전 답변 함수 내용과 동일
    relation_type = None; knob_names = None; reasoning = None
    pattern_map = {"POSITIVE_COUPLING": "positive", "INVERSE_RELATION": "inverse", "THRESHOLD_DEPENDENCY": "threshold", "NOTHING": "nothing"}
    try:
        pattern_match = re.search(r"-\s*pattern:\s*(\S+)", response, re.IGNORECASE)
        if pattern_match:
            raw_pattern = pattern_match.group(1).upper(); relation_type = pattern_map.get(raw_pattern)
            if relation_type is None and raw_pattern != "NOTHING": logger.info(f"WARN: 알 수 없는 LLM 패턴 '{pattern_match.group(1)}'")
        ...
        if knobs_match:
            try:
                ...
                if isinstance(parsed_list, list): knob_names = [str(item).strip() for item in parsed_list]
                else: logger.info(f"WARN: 파싱된 knobs가 리스트 아님: {knobs_match.group(1)}")
            except Exception as e: logger.info(f"WARN: knobs 문자열 파싱 실패: {knobs_match.group(1)} - {e}")
    ...
    except Exception as e: logger.info(f"❌ 오류: LLM 응답 파싱 중 문제 - {e}")
...
if __name__ == "__main__":
    ...
    for iteration in range(1, N_ITERATIONS + 1):
        ...
        base_ucb = UpperConfidenceBound(model=gp_model, beta=2.0) # exploration(beta가 커질수록 탐색 범위 넓어짐)
